          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '18px', padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
                ...
                <h3 style={{ color: 'white', fontSize: '13px', fontWeight: 700, margin: 0 }}>Top pays</h3>
              </div>
              {geoData.length === 0 ? (
                <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '12px', textAlign: 'center', padding: '16px 0' }}>Pas encore de données</p>
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '18px', padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '14px' }}>
                ...
                <h3 style={{ color: 'white', fontSize: '13px', fontWeight: 700, margin: 0 }}>Liens les plus cliqués</h3>
              </div>
              {topLinks.length === 0 ? (
                <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '12px', textAlign: 'center', padding: '16px 0' }}>Pas encore de données</p>
...
function OverviewPanel({ profile, onNavigate, onUpdate, onSave, hasChanges, saving }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {/* ══ LIGNE 2 : Raccourcis ══ */}
      <div style={{ display: 'grid', gridTemplateColumns: isMob ? '1fr' : 'repeat(2,1fr)', gap: '10px' }}>
        {quickActions.map(a => (
...
function Sidebar({ activeSection, onNavigate, profiles, activeProfileId, collapsed, onToggle, isMobile }) {
  ...
  // Desktop: 220px expanded / 64px collapsed
  // Mobile:  fixed drawer sliding in from left (always 260px wide, translated off/on screen)
...
export default function Dashboard() {
  ...
  // ── useQuery DOIT être avant les useEffects qui utilisent `profiles` ──
    ...
    <div style={{ minHeight: '100vh', display: 'flex', ...bgStyle, position: 'relative' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ position: 'sticky', top: 0, zIndex: 15, background: 'rgba(4,2,16,0.7)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: isMobile ? '10px 14px' : '10px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            {/* Logo — mobile uniquement (la sidebar est gérée par MobileNav) */}
            {isMobile && (
        ...
        {/* ── Mobile Nav (Tab Bar + Bottom Drawer) ── */}
        {isMobile && (
          <MobileNav
            activeSection={activeSection}
            onNavigate={setActiveSection}
