...
export default function StatsCard({ profileId }) {
  ...
  const metrics = [
    { icon: Eye, label: 'Vues', value: stats.views, color: '#6366f1', bg: 'rgba(99,102,241,0.1)' },
    { icon: MousePointer, label: 'Clics', value: stats.clicks, color: '#ff6b35', bg: 'rgba(255,107,53,0.1)' },
    { icon: TrendingUp, label: 'Taux', value: stats.views > 0 ? Math.round((stats.clicks / stats.views) * 100) + '%' : '0%', color: '#25D366', bg: 'rgba(37,211,102,0.1)' },
    { icon: Award, label: 'Top lien', value: stats.topPlatform ? stats.topPlatform.charAt(0).toUpperCase() + stats.topPlatform.slice(1) : '—', color: '#f7c948', bg: 'rgba(247,201,72,0.1)' },
  ];
    ...
    <div className="bg-card rounded-2xl border border-border p-4">
      ...
      {Object.keys(stats.clicksByPlatform).length > 0 && (
        <div style={{ marginTop: '12px', paddingTop: '12px', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
          <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginBottom: '8px' }}>Clics par plateforme</p>
          {Object.entries(stats.clicksByPlatform)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 3)
            .map(([platform, count]) => {
              const total = stats.clicks || 1;
