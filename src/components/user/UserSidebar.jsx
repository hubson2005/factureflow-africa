...
import { ChevronLeft, ChevronRight, Search, X, Lock, Crown } from 'lucide-react';
import { USER_NAV, USER_GROUPS } from '../../constants/userPlan';
import { PLAN_ORDER } from '../../constants/plans';

export default function UserSidebar({ activeSection, onNavigate, profile, plan, limits, collapsed, onToggle, isMobile }) {
  ...
  const filtered = USER_NAV.filter(n => !search || n.label.toLowerCase().includes(search.toLowerCase()));
      ...
      <div style={{ ...sidebarStyle, background: 'rgba(6,4,18,0.97)', backdropFilter: 'blur(24px)', borderRight: '1px solid rgba(255,255,255,0.07)', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: isMobile && !collapsed ? '8px 0 40px rgba(0,0,0,0.7)' : 'none' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', overflow: 'hidden' }}>
            ...
            {(!collapsed || isMobile) && (
              <div>
                ...
                <span style={{ color: limits.color, fontSize: '9px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{limits.emoji} {limits.label}</span>
              </div>
            )}
          </div>
        ...
        {(!collapsed || isMobile) && profile && (
          <div style={{ padding: '12px 14px', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            <div onClick={() => handleNav('settings', false)} style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '12px', padding: '10px 12px', display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '9px', background: `linear-gradient(135deg,${limits.color},${limits.color}99)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 700, color: 'white', flexShrink: 0, overflow: 'hidden' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ color: 'white', fontSize: '12px', fontWeight: 700, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{profile.display_name || 'Mon profil'}</p>
                {profile.username && <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '10px', margin: 0 }}>@{profile.username}</p>}
              </div>
        ...
        {collapsed && !isMobile && profile && (
          <div style={{ padding: '10px 0', display: 'flex', justifyContent: 'center', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            <div onClick={() => handleNav('settings', false)} style={{ width: '34px', height: '34px', borderRadius: '9px', background: `linear-gradient(135deg,${limits.color},${limits.color}99)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '13px', fontWeight: 700, color: 'white', overflow: 'hidden', cursor: 'pointer' }}>
              {profile.avatar_url ? <img src={profile.avatar_url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (profile.display_name?.[0]?.toUpperCase() || '?')}
            </div>
          </div>
        )}
            ...
            return (
              <div key={group.id} style={{ marginBottom: '4px' }}>
                ...
                {items.map(item => {
                  ...
                  return (
                    <button key={item.id} onClick={() => handleNav(item.id, locked)}
                      title={collapsed && !isMobile ? item.label + (locked ? ` (${lockLabel})` : '') : ''}
                      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: collapsed && !isMobile ? 0 : '10px', padding: collapsed && !isMobile ? '10px 0' : '9px 10px', borderRadius: '11px', border: 'none', background: isActive && !locked ? 'rgba(99,102,241,0.18)' : 'transparent', cursor: locked ? 'default' : 'pointer', opacity: locked ? 0.45 : 1, justifyContent: collapsed && !isMobile ? 'center' : 'flex-start', position: 'relative', marginBottom: '2px', transition: 'background 0.12s' }}>
                      {isActive && !locked && <div style={{ position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)', width: '3px', height: '20px', background: 'linear-gradient(180deg,#6366f1,#8b5cf6)', borderRadius: '0 3px 3px 0' }} />}
                      <div style={{ width: '30px', height: '30px', borderRadius: '9px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: isActive && !locked ? 'rgba(99,102,241,0.25)' : 'transparent' }}>
                        {locked ? <Lock size={14} color="rgba(255,255,255,0.3)" /> : <item.icon size={15} color={isActive ? '#a78bfa' : 'rgba(255,255,255,0.45)'} />}
                      </div>
                      {(!collapsed || isMobile) && (
                        <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '6px', minWidth: 0, overflow: 'hidden' }}>
                          <span style={{ color: isActive && !locked ? 'white' : 'rgba(255,255,255,0.55)', fontSize: '12.5px', fontWeight: isActive && !locked ? 700 : 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{item.label}</span>
                          {locked && <span style={{ flexShrink: 0, background: lockColor + '18', border: '1px solid ' + lockColor + '44', borderRadius: '5px', padding: '1px 5px', fontSize: '8px', color: lockColor, fontWeight: 700, textTransform: 'uppercase' }}>{lockLabel}</span>}
                        </div>
                      )}
                      {locked && collapsed && !isMobile && <div style={{ position: 'absolute', top: '6px', right: '6px', width: '10px', height: '10px', borderRadius: '50%', background: lockColor, border: '1px solid rgba(0,0,0,0.5)' }} />}
                    </button>
                  );
                })}
              </div>
        ...
        {(!collapsed || isMobile) && (
          <div style={{ padding: '12px 14px', borderTop: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
            <div style={{ background: limits.color + '18', border: '1px solid ' + limits.color + '44', borderRadius: '10px', padding: '8px 10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
                ...
                {limits.price && <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: '9px', marginLeft: 'auto' }}>{limits.price}</span>}
              </div>
