...
export default function EventDashboard() {
  ...
  const handleRSVP = () => {
    const message = `Bonjour 👋🏾\nJe confirme ma présence à votre événement.%0A%0A👤 Nom : ${form.name}%0A📞 Téléphone : ${form.phone}%0A👥 Nombre de personnes : ${form.guests}`;

    window.open(
      `https://wa.me/${eventData.whatsapp}?text=${message}`,
      "_blank"
    );
  };
        ...
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3">
           ...
           <p className="text-zinc-400 mt-2">
            Gérez votre événement, vos RSVP et vos invités depuis un seul dashboard.
          </p>
        </div>

        <button className="bg-orange-500 hover:bg-orange-600 transition px-5 py-3 rounded-xl font-semibold flex items-center gap-2">
          <QrCode size={18} />
          Télécharger QR Code
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-[#1a1c21] border border-white/10 rounded-2xl p-6">
          ...
          <div className="mt-4">
            ...
            <input
              ...
              onChange={(e) =>
                setEventData({ ...eventData, whatsapp: e.target.value })
              }
              className="w-full bg-[#22252c] border border-white/10 rounded-xl px-4 py-3 outline-none"
            />
          </div>
        ...
        <div className="bg-[#1a1c21] border border-white/10 rounded-2xl overflow-hidden">
          ...
          <div className="p-5 space-y-4">
            ...
            <button className="w-full bg-orange-500 hover:bg-orange-600 transition rounded-xl py-3 font-semibold flex items-center justify-center gap-2">
